...
export default function ISSGlobe() {
  ...
  const Globe = () => {
    ...
    const latLonToXYZ = (lat: number, lon: number, radius: number): [number, number, number] => {
      const phi = (90 - lat) * (Math.PI / 180);
      const theta = (lon + 180) * (Math.PI / 180);
      const x = -radius * Math.sin(phi) * Math.cos(theta);
      const y = radius * Math.cos(phi);
      const z = radius * Math.sin(phi) * Math.sin(theta);
      return [x, y, z]; // ✅ Return a tuple, not array
    };

    const issPos: [number, number, number] = latLonToXYZ(iss.latitude, iss.longitude, 2.05);
